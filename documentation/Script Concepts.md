...
When a string is contextually localized with a localization object, then there's one root object (either a Scope or [Localization Environment(loc_objects_documentation.md#localization_environment)].
In general this object can only be used for two purposes: Accessing other objects and getting the current date.

The documentation for the different localization objects can be found at Localization Objects.
...
from the event scope of that context. For example, an effect that creates a trade route between two countries `FROM` and `THIS` could be localized with:
`LOC_KEY: "Creates a trade route between [FROM.GetName] and [THIS.GetName]"`. For more information on how the scope accessors `FROM`, `THIS`, `PREV` and `ROOT` work, see the Scope object documentation.
...
and raw localization strings (which can include Contextual Localization), giving the following allowed options:
* A localization tag.
* A string.
* A formatter and it's associated tag.

Documentation for the existing formatters can be found at Localization Formatters.
...
# Using a loc string with contextual localization
